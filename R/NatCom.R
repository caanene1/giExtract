...
files <- list.files("/Users/chineduanene/Documents/OneDrive/NatCom/Data/CNESCC_JPF_sf8-PWF/cubes",
                    pattern = ".jpg")
...
obj$Group <- gsub("B DIF", "DIFF", obj$Group)
...
### Begin analysis #############
### Read files
obj <- read.csv("~/Documents/OneDrive/NatCom/Data/context-NatCom.csv")
predict <- read.csv("~/Documents/OneDrive/NatCom/Data/Predictions_context-NatCom.csv.csv")

# Merge the data
final <- merge(obj, predict, by = "Name")
...
# Prepare the Discovery and training data
...
# Bind the two datasets for easy processing
...
# Now filter to make sure the two rules above are meet
